...
// Demo credentials (in production, this goes through your backend API)
...
function handleLogin(event) {
    ...
    setTimeout(() => {
        const match = VALID_CREDENTIALS.find(
            c => (c.username === email.toLowerCase()) && c.password === password
        );
        
        if (match) {
            // Success — save session
            const sessionData = { loggedIn: true, username: match.username, remember, timestamp: Date.now() };
            ...
            // Fail — show error
            loginBtn.classList.remove('loading');
            errorTextEl.textContent = 'Invalid email or password. Please try again.';
